...
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1'>
  <tr class='topbg'>
    <td height='30' align="center" ><strong>开支记录-录入开支</strong></td>
  </tr>
</table>
...
if Requesta("submessage")<>"" then
	...
	if Requesta("submessage")="提交" then
		...
		Rs.close
		Sql="Select * from ourmoney where PayMethod='" & Requesta("PayMethod") &"' and Amount=" & Amount & " and DateDiff('dddd',PayDate,'" & Requesta("PayTime") & "')=0"
...
<form name="formSignUp" method="post" action="" onSubmit="return check(this);">
                        <tr valign="middle"> 
                          <td colspan="2" class="tdbg">&nbsp;</td>
                        </tr>
                        <tr> 
                          <td width="33%" align="right" class="tdbg"> 
                          姓名：</td>
                          <td width="67%" class="tdbg"><input type="text" name="Name" value="<%=Requesta("name")%>"></td>
                        </tr>
                        <tr> 
      ...
      <td align="right" class="tdbg"> 支付方式：</td>
                          <td class="tdbg">                            <select name="PayMethod" id="PayMethod">
                              ...
                              <option value="财付通支付"<%if PayMethod="财付通支付" then%> selected<%end if%>>财付通支付</option>
      ...
      <td align="right" class="tdbg"> 金额：</td>
                          <td class="tdbg">                            <input type="text" name="Amount" value="<%=Amount%>">
        <font color="#FF0000">记录支出时</font>，金额填负数即可。如：-200</td>
                        </tr>
                        <tr> 
                          <td align="right" class="tdbg"> 
                          订单号：</td>
                          <td class="tdbg">                            <input type="text" name="Orders" value="<%=Requesta("Orders")%>">
        可不填 </td>
                        </tr>
                        <tr> 
                        ...
                        <tr>
                          <td align="right" class="tdbg">是否是内部转帐：</td>
                          <td class="tdbg"> 
<%
...
%>
                            ...
                            是 <br>
        (比如将公司的现金存到公司帐号，请选择“是”，这样不计入成本。</td>
                        </tr>
                        <%if len(AlertMessage)>5 then%>
                        <tr align="center"> 
                          <td colspan="2" align="right" class="tdbg"><%=AlertMessage%></td>
                        </tr>
                        <%end if%>
                        <tr> 
                          <td align="right" class="tdbg">&nbsp;                          </td>
                          <td class="tdbg">                            <input type="submit" name="submessage" value="<%=submessage%>">
                            <input type="button" name="Submit2" value="清 除">
                          <input type="hidden" name="id" value="<%=Requesta("id")%>"></td>
                        </tr>
                        <tr>
                          <td colspan="2" align="right" class="tdbg">&nbsp;</td>
                        </tr>
</form>
                      </table>
                      记录所有开支， 注意金额填负数。在查看流水帐　功能中，会直接显示收入、 支出、利润<br>
                      <br>
