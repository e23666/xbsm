...
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1'>
  <tr class='topbg'>
    <td height='30' align="center" ><strong>查看流水帐</strong></td>
  </tr>
</table>
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1' class='border'>
  <tr class='tdbg'>
    <td width='91' height='30' align="center" ><strong>管理导航：</strong></td>
    <td width="771"><a href="<%=SystemAdminPath%>/billmanager/incount.asp" target="main">用户入款</a>| 
...
if isDate(StartTime) then BaseSql=BaseSql & " and datediff("&PE_DatePart_D&",'" & StartTime & "',PayDate)>=0"
if isDate(EndTime) then   BaseSql=BaseSql & " and datediff("&PE_DatePart_D&",PayDate,'" & EndTime & "')>=0"
if PayMethod<>"" then BaseSql=BaseSql & " and PayMethod like '%" & PayMethod & "%'"

Sql="Select top 1000 a.*,b.u_id from (ourmoney a inner join userdetail b on b.u_name=a.username) where " & BaseSql & " order by id desc"
...
Sql4="Select sum(Amount) as TMoney from ourmoney where Amount>0 and ismove=0 and datediff("&PE_DatePart_D&",PayDate,"&PE_Now&")=0"
Sql5="Select sum(Amount) as MMoney from ourmoney where Amount>0 and ismove=0 and dateDiff("&PE_DatePart_M&",PayDate,"&PE_Now&")=0"
...
%> 
        <tr> 
          ...
          <td class="tdbg"> 
            <%=rs("Name")%>           </td>
          <td class="tdbg"> 
            <a href="../usermanager/detail.asp?u_id=<%=Rs("u_id")%>" target="_blank"><font color="#0000FF"><%=rs("UserName")%></font></a></td>
          <td align="center" nowrap bgcolor="#EAF5FC" class="tdbg" width="4%"><a href="../chguser.asp?module=chguser&username=<%=rs("UserName")%>"><font color="#0000FF">代管</font></a></td>
          <td class="tdbg"> 
            <%=rs("Orders") & "&nbsp;"%></td>
          <td class="tdbg"> 
            <%=rs("PayDate")%></td>
