...
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1'>
  <tr class='topbg'>
    <td height='30' align="center" ><strong>企业邮局续费管理(今天日期：<%=formatDateTime(now(),2)%>)</strong></td>
  </tr>
</table>
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1' class='border'>
  <tr class='tdbg'>
    <td width='91' height='30' align="center" ><strong>管理导航：</strong></td>
    <td width="771"><a href="../sitemanager/c_charge.asp">主机续费通知</a> | <a href="c_chage.asp">邮局续费通知</a> | <a href="../domainmanager/c_charge.asp">域名续费通知</a></td>
  </tr>
</table>
<br>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="border">
  ...
  Action=requesta("Act")
if Action="" then
	sql="SELECT dateDiff("&PE_DatePart_D&",'"&now()&"',dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate)) as exDate, productlist.P_proId, productlist.p_name, mailsitelist.m_bindname,  mailsitelist.m_buydate, mailsitelist.m_expiredate, UserDetail.u_name,mailsitelist.m_years,dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate) as extime,mailsitelist.m_ownerid FROM (mailsitelist INNER JOIN productlist ON mailsitelist.m_productId = productlist.P_proId) INNER JOIN UserDetail ON mailsitelist.m_ownerid = UserDetail.u_id WHERE dateDiff("&PE_DatePart_D&",'"&now()&"',dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate))>=0 and dateDiff("&PE_DatePart_D&",'"&now()&"',dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate))<=30"
else
	Exddate=requesta("Exddate")
	if not isnumeric(Exddate) then
		url_return "输入值只能为整数。",-1
	end if
	sql="SELECT dateDiff("&PE_DatePart_D&",'"&dateadd("&PE_DatePart_Y&"()&"',dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate)) as exDate, productlist.P_proId, productlist.p_name, mailsitelist.m_bindname,  mailsitelist.m_buydate, mailsitelist.m_expiredate, UserDetail.u_name,mailsitelist.m_years, UserDetail.u_name,mailsitelist.m_years,dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate) as extime,mailsitelist.m_ownerid FROM (mailsitelist INNER JOIN productlist ON mailsitelist.m_productId = productlist.P_proId) INNER JOIN UserDetail ON mailsitelist.m_ownerid = UserDetail.u_id WHERE dateDiff("&PE_DatePart_D&",'"&dateadd("&PE_DatePart_Y&"()&"',dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate))>=0 and dateDiff("&PE_DatePart_D&",'"&dateadd("&PE_DatePart_Y&"()&"',dateadd("&PE_DatePart_Y&",mailsitelist.m_years,mailsitelist.m_buydate))<="&Exddate
end if
...
%>
  <tr> 
    <td dateadd("&PE_DatePart_Y&"rap class="tdbg"><%=rs("m_bindname")%></td>
