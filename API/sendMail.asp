...
sub cleanAPILOG()
	conn.execute("delete from ActionLog where LogType='API' and dateDiff("&PE_DatePart_D&",addTime,"&PE_Now&")>1")
end sub
...
sub pastnote()
	...
	Subject="邮局续费"
	mobnumC=""
	sqlmail="Select a.m_bindname,a.m_ProductId,a.m_buydate,a.m_years,dateDiff("&PE_DatePart_D&","&PE_Now&",dateadd("&PE_DatePart_Y&",m_years,m_buydate)) as day_level,b.u_email,b.u_name,b.msn_msg from (mailsitelist a inner join userdetail b on a.m_ownerid = b.u_id) where m_free=0 and dateDiff("&PE_DatePart_D&","&PE_Now&",dateadd("&PE_DatePart_Y&",m_years,m_buydate)) in ("& day_level_1&","& day_level_2 &","& day_level_3 &")"
...
'短信通知
	...
	'sqlserver="Select [id],[name],[email],allocateip,[years],moneypermonth,alreadypay,StartTime,[fax] as msn_msg from hostrental where start="&PE_True&" and dateDiff("&PE_DatePart_D&","&PE_Now&",dateAdd("&PE_DatePart_M&",AlreadyPay,starttime)) in ("& day_level_1&","& day_level_2 &","& day_level_3 &")"
	If isdbsql Then
...
sub DeleteExpiredData()
   ...
   '域名
   sql2="delete from mailsitelist where dateDiff("&PE_DatePart_D&",dateadd("&PE_DatePart_Y&",m_years,m_buydate),"&PE_Now&")>30"
   'mssql
