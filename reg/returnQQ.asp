...
session("qq_nickname")=qc.GetUserName(userinfo)(0)
   ...
   '登陆状态操作
    if session("u_sysid")<>"" then
		...
		case "1"  '老用户绑定QQ
		'response.Write("绑定QQ"&rs.eof)
			if rs.eof then  '检查是否已经有绑定
...
<body>
<div class="main">
    	<h2 class="green" style="margin:0px;">QQ快速登陆注册(完善联系方式)</h2>
    ...
    <tr style="display:none">
      ...
      <td><input name="qq_msg" type="text" class="inputxt" id="qq_msg"   ignore="ignore"   maxlength="30"    datatype="qq" errormsg="请输入正确的QQ号码！！" /></td>
      <td><div class="Validform_checktip">可以输入您的QQ号码</div></td>
    </tr>
    <tr>
                    <td colspan="4"  ><input type="submit" name="button" id="button" value="提交" />
                    <a href="?action=noreg" target="_top" onclick="return confirm('注意请先完善联系方式后再购买，否则无法收到相应续费购买等信息')">以后再说。我自行到用户中心完善</a></td>
                </tr>
  </table>
</form>
...
response.Write("<script>alert('参数错误请重新登陆');window.opener.closeChildWindow();</script>")
response.end()
else
