...
Class AlipayNotify
	...
	Public Function VerifyReturn()
		...
		If IsArray(sParaTemp) Then
			...
			sResponseTxt = "true"
			If Request("notify_id") <> "" Then
				sResponseTxt = GetResponse(Request("notify_id"))
			End If
			...
			'判断VerifyNotify是否为ture，生成的签名结果mysign与获得的签名结果sign是否一致
			'responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
			'mysign与sign不等，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
			If mysign = Request("sign") And sResponseTxt = "true" Then
				VerifyReturn = true
			Else
				VerifyReturn = false
			End If
