<%
	...
	Private Function ASP_ConvertToWordArray(sMessage)
		...
		Do Until lByteCount >= lMessageLength
			...
			lWordArray(lWordCount) = lWordArray(lWordCount) Or ASP_LShift(Asc(Mid(sMessage, lByteCount + 1, 1)), lBytePosition)
	...
	Public Function ASP_MD5(sMessage)
		...
		x = ASP_ConvertToWordArray(sMessage)
		...
		if aspmd5mode="16" then
			ASP_MD5=LCase(ASP_WordToHex(b) & ASP_WordToHex(c)) 
		else
...
Function md5_16(M_)
md5_16=Mid(ASP_MD5(M_),9,16) 
End Function

'使用方法是 md5 ("字符串") 下面是使用示范
'Response.Write "md5('a')的加密结果为[" & md5 ("a") & "]<br>"
