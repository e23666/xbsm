...
SessionName = "caplogin"	'replace(replace(request("codename"),"'",""),"u_","")'Where store your secure code
...
strip = request.servervariables("HTTP_CDN_SRC_IP")
If Not isip(strip) Then
	Session(SessionName) = secureCode
Else
	Call setapplication_("cap" & strip , secureCode)
End If
