...
if whoistoken="" then die "服务器忙请稍后1!!"
if not isnumeric(t&"") then  die "服务器忙请稍后2!!"
newtimer=ToUnixTime(now(),+8)
if clng(newtimer-t)>8 then  die "服务器忙请稍后3!!"
...
if left(returnstr,3)="200" then
	...
	for i=0 to ubound(returnarr)
		...
		if trim(domainTypelist)<>"" then
			
			if i=0 then
				lookimg="<img src=""/Template/Tpl_01/images/drcr.gif"" width=""17"" height=""17"">"
				looktitle="<strong><font color=""#669966"">以下域名可以注册</font></strong>"
			elseif i=1 then
				lookimg="<img src=""/Template/Tpl_01/images/20050703091647167.gif"" width=""14"" height=""11"">"
				looktitle="<strong><font color=""#CC0000"">以下域名已经被注册</font></strong>"
				session("esymok")="ok"
			else
				lookimg="<img src=""/Template/Tpl_01/images/20050703091648248.gif"" width=""14"" height=""11"">"
				looktitle="<strong><font color=""#660099"">以下域名因网络原因无法查到正确的状态</font></strong>"
			end if
    ...
    <tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="3">
          <tr>
            ...
            <td width="93%" class="14pxfont"><%=looktitle%></td>
          </tr>
        </table></td>
    </tr>
    <tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		for each domainlist in split(domainTypelist,",")
			...
			if instr(domainlist,".")>0 then
					...
					If p_proid="domcn" Then
						href="dmbuyMore.asp"'?domain=" & domainlist
					else
						href="dmbuyMore.asp"'?domain=" & domainlist
					end if
		...
		%>
          <tr>
            <td width="15%" align="right"><%=lookimg%></td>
            <td width="42%" nowrap="nowrap"><%
	  			othersuffixstr=".tw|.cd|.hk|.ph|.us|.tel"
				getdomainsuffix=trim(mid(domainlist,instr(domainlist,".")))
				otherspd=true
				'if instr("|"& othersuffixstr &"|","|"& getdomainsuffix &"|")>0 then
					'otherspd=false
				'end if
				if otherspd and not ishz and i=0 then
	  		 %>
              <input type="checkbox" name="domainsset" value="<%=domainlist%>" checked>
              <%	end if
				if i=0 then
			%>
              <a href=#### onclick="getTopost('<%=href%>','<%=domainlist%>')"><span style="color: #0066CC"><%=domainlist%></span></a><%if right(domainlist,7)=".gov.cn" then%><a href="/faq/domgov.asp" target="_blank">【政府域名注册须知】</a><%end if%><%if right(domainlist,3)=".cn" then%><a href="/faq/domcn.asp"  target="_blank">【CN域名注册须知】</a><%end if%>
              <%  
				elseif i=2 then
				%>
              尝试注册:<a href=#### onclick="getTopost('<%=href%>','<%=domainlist%>')"><span style="color: #660099"><%=domainlist%></span></a>
              <%
				else
				%>
              <a href="view.asp?domain=<%=server.URLEncode(domainlist)%>" target="_blank"><span style="color: #333333"><%=domainlist%></span></a>
              <%
				end if
			%>
            </td>
            <td width="43%" nowrap="nowrap"><%if i=0 then%>
              <a href=#### onclick="getTopost('<%=href%>','<%=domainlist%>')"><img src="/Template/Tpl_01/images/button_r_domain_15.gif" width="80" height="21" border="0"></a>&nbsp;
              <%if not otherspd then response.write "<font color='#cccccc'>特殊域名.只能单独注册</font>"%>
              <%end if%>
            </td>
          </tr>
          <tr>
            <td background="/Template/Tpl_01/images/sens_mainbg2.gif" colspan="3" height="1"></td>
          </tr>
          <%
			end if
		 next
        
			 if i=0 and not ishz then
	 %>
          <tr>
            <td colspan="3" align="center"><input name="allregsub" type="button" value="批量注册" onClick="return doallsub(this.form)">
            </td>
          </tr>
          <%end if%>
        ...
        <%if i=0 then%>
        <input type="hidden" name="ajaxFree" />
        <%end if%></td>
    </tr>
    <%
  		end if
...
%>
    <tr>
      <td align="center"><img src="/Template/Tpl_01/images/new/err101.gif" border="0" /><br />
        出现错误:<%=wd.resultstr%> </td>
    </tr>
    <%
end if%>
