...
elseif act="add" then
	...
	if left(returnstr,3)="200" then
        ...
        conn.Execute(Sql)
		'Response.write "<div align=center><img src=/images/process.jpg><br><br>域名实时注册成功，该域名将在24小时内生效，现在可进入管理中心-业务管理-<a href='/manager/domainmanager/'>域名管理</a>对此域名进行相关设置</div>"
			alterstr="域名实时注册成功，该域名将在24小时内生效<br>" & _
					"现在可进入 管理中心-业务管理-域名管理 对此域名进行相关设置"
		call doupfile(strdomain,"http://"&request.ServerVariables("HTTP_HOST")&"/manager/config/echo.asp?str="& server.urlEncode(alterstr)&"&p=r")			
				...
				echoString "域名注册失败, "& showerrmsg(returnstr) ,"e"
	end if
end if
...
<div class="Style2009"> 
  ...
  <tr class="tdbg">
  <td width="29%" align="right" nowrap>订单号:</td>
  <td width="71%" class="tdbg"><%=orderID%></td>
  </tr>
  <tr class="tdbg">
  <td align="right" nowrap>产品名:</td>
  <td class="tdbg"><%=strDomain%></td>
  </tr>
  <tr class="tdbg">
  <td align="right" nowrap>下单日期:</td>
  <td class="tdbg"><%=regDate%></td>
  </tr>
   <tr>
  <td align="right" nowrap class="tdbg">购买年限:</td>
  <%if act="mod" then%>
  <td class="tdbg">
  ...
  <span class="STYLE4"><a href=# onclick="javascript:modifyyears('<%=request("script_name")%>',<%=d_id%>);">[确定修改]</a>
  <a href="<%=request("script_name")%>?id=<%=d_id%>">[取消修改]</a></span> </td>
  <%else%>
  <td class="tdbg"><strong><%=years%>年&nbsp;</strong> <a href="<%=request("script_name")%>?act=mod&id=<%=d_id%>" class="STYLE4">[修改年限]</a></td>
  <%end if%>
  </tr>
  <tr class="tdbg">
  <td align="right" nowrap>总价格:</td>
  ...
  <input type="button" name="button" id="button" value="编辑订单" onclick="javascript:location.href='modifyorder.asp?domainid=<%=d_id%>';" />
  <%end if%>
